...
import {activityOptions, booleanOptions, cutOptions, statusOptions, verticalOptions} from 'helpers/options'
import {groupOrdersSchema, temporaryOrderSchema, temporaryOrderSchema2} from 'helpers/yup'
import {
    useActions,
    useAdd,
    useData,
...
const Index = () => {
    ...
    const {
        ...
        resolver: yupResolver(detail?.is_list ? temporaryOrderSchema2 : temporaryOrderSchema)
    })
    ...
    useEffect(() => {
        if (detail && updateId) {
            resetEdit({
                ...
                is_vertical: detail?.is_vertical,
    ...
    }, [x, y, glueSquare, selectedOrders, watch('has_addition')])

    const onSubmit = (data: ISearchParams) => {
        if ((orders?.length || 0) <= 0) {
            showMessage('Select at least one order', 'error')
        }
        // else if (areAllFieldsPresent(orders) && orders?.every(order => order.is_list)) {
        ...
        <>
                                        ...
                                        <div className="grid gap-md">
                                            ...
                                            <div className="span-4">
                                                <Select
                                                    ...
                                                    value={getSelectValue(verticalOptions as unknown as ISelectOption[], order?.is_vertical || verticalOptions[0].value)}
                                                    defaultValue={getSelectValue(verticalOptions as unknown as ISelectOption[], order?.is_vertical || verticalOptions[0].value)}
                                                />
                                            </div>
                                            ...
                                            >
                                                ...
                                                <div className="span-4 flex gap-md align-end justify-start">
                                                    <input
                                                        id={activityOptions[6].value?.toString() as string}
                                                        type="checkbox"
                                                        className="checkbox"
                                                        checked={order?.stages_to_passed?.includes(activityOptions[6].value?.toString() as string) || false}
                                                    />
                                                    <p className="checkbox-label">
            ...
            >
                    ...
                    onSubmit={
                        handleEditSubmit((data) => {
                            const newData = {
                                ...
                                stages_to_passed: Object.keys(data).filter((key) => data[key as keyof typeof data] === true && key !== 'length' && key !== 'is_vertical')?.reverse()
                            }

                            update(newData).then(async (data: any) => {
                                updateOrder({
                                    ...data as any,
                                    id: +(updateId || 0)
                                })
                ...
                >
                    ...
                    <div
                        className="span-12 flex gap-md flex-wrap"
                        style={{marginTop: '.75rem', marginBottom: '1.5rem'}}
                    >
                        <div className="span-4 flex gap-md align-end justify-start">
                            <input
                            ...
                            <Controller
                                ...
                                render={({field: {value, onChange, ref, onBlur}}) => (
                                    <Select
                                        ...
                                        label={t('Joylashuv')}
                                        options={verticalOptions as unknown as ISelectOption[]}
                                        value={getSelectValue(verticalOptions as unknown as ISelectOption[], value)}
                                        defaultValue={getSelectValue(verticalOptions as unknown as ISelectOption[], value)}
