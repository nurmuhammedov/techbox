...
import {countrySchema} from 'helpers/yup'
...
const Index = () => {
	...
	const {mutateAsync, isPending: isAdding} = useAdd('products/countries')
	const {mutateAsync: update, isPending: isUpdating} = useUpdate('products/countries/', updateId)
	const {data: detail, isPending: isDetailLoading} = useDetail<{
		name: string,
		id: number
	}>('products/countries/', updateId)
	...
	return (
		<>
			...
			<Modal title="Add country" id="countries" style={{height: '25rem'}}>
				<Form
			...
			<DeleteModal endpoint="products/countries/" onDelete={() => refetch()}/>
		</>
	)
}
