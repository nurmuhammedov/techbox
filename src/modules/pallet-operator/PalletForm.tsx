...
import {Button, Card, Diagram, Form, Input, MaskInput, NumberFormattedInput, PageTitle} from 'components/index'
...
const PalletForm: FC<IProps> = ({retrieve = false, type}) => {
	...
	useEffect(() => {
		if (retrieve && pallet) {
			let defaultEndCount = pallet.end_count
			if (type === 'flex') defaultEndCount = pallet.pallet_count_after_flex ?? pallet.end_count
			else if (type === 'glue') defaultEndCount = pallet.pallet_count_after_gluing ?? pallet.end_count
			else if (type === 'bet') defaultEndCount = pallet.pallet_count_after_bet ?? pallet.end_count

			reset({end_count: defaultEndCount?.toString() || ''})
		} else if (pallet) {
			reset({end_count: ''})
		}
	}, [pallet, retrieve, reset, type])
